...
export const CommonFieldStoryProps = (uiType: string, isElement = false): ArgTypes => {
	...
	return {
		...
		label: {
			description: `A name/description of the purpose of the form element which may include an optional sub-label and popover feature.<br>If string is provided, the entire label will be rendered.<br>If object is provided:<ul><li>mainLabel: Primary text to display.</li><li>subLabel: Secondary text to display below the mainLabel.</li><li>hint.content: Displays an info icon and brings up the content as a popover on click.</li></ul>`,
			table: {
				type: {
					summary:
...
export const COLUMNS_ARG_TYPE: ArgTypes = {
	columns: {
		description: `Specifies the number of columns to be span across in desktop / tablet / mobile viewports. If an array is specified, the format is as such <code>[startCol, endCol]</code>.<br><br>The system automatically determines which grid layout to use based on the properties you provide:<br>V2 Grid System is applied when you use <code>desktop, tablet, or mobile</code> properties<br>V3 Grid System is applied when you use any of the properties <code>xxs, xs, sm, md, lg, xl, or xxl</code><br><br>For <code>v2</code> version:<br>Permitted values:<br>Desktop: <code>1 - 12</code> and <code>1 - 13</code> if specifying a range.<br>Mobile: <code>1 - 4</code> and <code>1 - 5</code> if specifying a range.<br><br>Settings are applied by similar to how <code>@max-width</code> works: if <code>desktop</code> is not specified, <code>tablet</code> will be used for desktop and tablet, if <code>tablet</code> is also not specified, <code>mobile</code> will be used for all screen sizes.<br><br>If all column settings are not specified, element will span across a single column.<br><br>For <code>v3</code> version:<br>Permitted values:<br>xxs-md: <code>1-8</code> or <code>1-9</code> if specifying a range.<br>lg-xxl: <code>1-12</code> or <code>1-13</code> if specifying a range.<br><br>Settings are applied by similar to how <code>@max-width</code> works: if <code>lg-xxl</code> is not specified, <code>xxs-md</code> will be used for all screen sizes.<br><br>If all column settings are not specified, element will span across a single column.<br><br>`,
		table: {
			type: {
