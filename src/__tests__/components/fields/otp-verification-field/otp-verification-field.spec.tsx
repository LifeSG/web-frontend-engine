...
const getSendOtpButton = () => screen.getByRole("button", { name: /send otp/i });
const getVerifyOtpButton = () => screen.getByRole("button", { name: /verify/i });
...
describe(UI_TYPE, () => {
	...
	it("should be able to render the phone-number OTP field", () => {
		renderComponent();
		expect(screen.getByLabelText(COMPONENT_LABEL, { exact: false })).toBeInTheDocument();
	});

	it("should be able to render the email OTP field", () => {
		renderComponent({ validation: [{ "otp-type": "email" }] });
		expect(screen.getByLabelText(COMPONENT_LABEL, { exact: false })).toBeInTheDocument();
	});
	...
	it("should call the send OTP API with the contact number when send OTP button is clicked", async () => {
		...
		await waitFor(() => {
			expect(postSpy).toHaveBeenCalledWith(
				SEND_OTP_URL,
				expect.objectContaining({ contact: expect.any(String) }),
				expect.anything()
			);
		});
	});
	...
	it("should show send OTP error on API failure", async () => {
		jest.spyOn(AxiosApiClient.prototype, "post").mockRejectedValue(new Error("network error"));

		renderComponent();
		fireEvent.click(getSendOtpButton());

		await waitFor(() => {
			expect(screen.getByText(ERROR_MESSAGES.OTP_VERIFICATION.SEND_FAILED)).toBeInTheDocument();
		});
	});
	...
	it("should show verify OTP error on API failure", async () => {
		...
		await waitFor(() => {
			expect(screen.getByText(ERROR_MESSAGES.OTP_VERIFICATION.VERIFY_FAILED)).toBeInTheDocument();
		});
	});
	...
	it("should support required validation - reject submission when not verified", async () => {
		renderComponent({ validation: [{ "otp-type": "phone-number" }, { required: true }] });
	...
	it("should show MUST_BE_VERIFIED error when only send OTP was completed but not verify", async () => {
		jest.spyOn(AxiosApiClient.prototype, "post").mockResolvedValue({ transactionId: "txn-123" });
		...
		await waitFor(() => {
			expect(screen.getByText(ERROR_MESSAGES.OTP_VERIFICATION.MUST_BE_VERIFIED)).toBeInTheDocument();
		});
		expect(SUBMIT_FN).not.toHaveBeenCalled();
	});
