import { OneMapGeocodeInfo, OneMapSearchBuildingResult } from "../services/types";
...
export class LocationHelper {
	private static _getCurrentLocation(
		timeout: number,
		maximumAge: number,
		disableErrorPromptOnApp: boolean
	 ...
	 */
	public static async getCurrentLocation(options: IGetCurrentLocationOptions = {}): Promise<ILocationCoord> {
		...
		do {
			...
			try {
				return await this._getCurrentLocation(timeout, maximumAge, disableErrorPromptOnApp);
			} catch (error) {
	...
	public static hasGotAddressValue = (value?: string): boolean => {
		const lowercased = value?.toLowerCase();
		return !!value && lowercased !== "nil" && lowercased !== "null";
	};

	public static formatAddressFromGeocodeInfo = (
	...
	public static isOneMapSearchBuildingResults = (
		geoCodeInfo: OneMapGeocodeInfo | OneMapSearchBuildingResult
	 ...
	 */
	public static distanceBetweenTwoPoints = (coord1: ILocationCoord, coord2: ILocationCoord) => {
	 ...
	 */
	public static checkIsWithinRadius = (coord1: ILocationCoord, coord2: ILocationCoord, radius: number) => {
