...
function normaliseCssValue(key: string, value: string | number): string {
	if (typeof value === "number" && numericCssStyleProperties.includes(key as keyof CSSStyleDeclaration)) {
		return `${value}px`;
	}
 ...
 */
export const useApplyStyle = <T extends HTMLElement>(ref: RefObject<T>, styles: Record<string, StyleValue>) => {
